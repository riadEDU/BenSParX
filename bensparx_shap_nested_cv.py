"""
BenSParX — Nested-CV SHAP Analysis (Reviewer #2, Option B)
=============================================================

Purpose
-------
Reviewer #2 asked that the SHAP analysis in Section 4.6 be grounded entirely
in models evaluated on held-out data, rather than a single refit on the full
dataset. This script implements that: it re-runs the outer loop of the
nested cross-validation exactly as described in Section 3.6 (group-wise,
speaker-level 10-fold outer CV; 5-fold inner CV for hyperparameter tuning),
performs Mann-Whitney U feature selection *inside* each outer training fold
only, fits Random Forest per fold, and computes SHAP values ONLY on each
fold's held-out test partition. Because 10-fold CV visits every sample
exactly once as a test case, concatenating these per-fold, out-of-fold SHAP
values yields one honest SHAP value per feature per sample for the entire
dataset, with zero leakage.

It reproduces:
  - Figure 4  : SHAP beeswarm summary plot (pooled out-of-fold values)
  - Figure 5  : SHAP waterfall plot for a representative PD instance
  - Figure 6  : SHAP waterfall plot for a representative HC instance
  - Supplementary Table : feature-rank stability across the 10 outer folds
                          (mirrors the fold-range style already used in
                          Table 6), addressing the reviewer's request to
                          "show the variability of feature rankings across
                          folds."

Usage
-----
    python bensparx_shap_nested_cv.py --data your_features.csv \
        --label-col label --group-col participant_id --outdir ./shap_out

Your CSV should have:
  - one row per 10-second audio segment (900 rows in the paper)
  - a label column: 1 = PD, 0 = HC (values/name configurable via --label-col
    and --pos-label)
  - a group column: participant/speaker ID, used for the group-wise split
    so that no participant's segments appear in both train and test
  - all remaining numeric columns treated as acoustic features (MFCC,
    jitter, shimmer, formants, pulse, harmonicity, intensity, pitch, etc.)

If --data is omitted, the script runs on a synthetic demo dataset that
mimics the structure of BenSParX (900 rows, 120 speakers, 71 features)
purely so you can confirm the pipeline runs end-to-end before pointing it
at the real data.

Notes on methodological choices (documented so they can go straight into
the rebuttal / methods text):
  1. Feature selection (Mann-Whitney U, alpha=0.05) is re-run independently
     within each outer training fold — never on the test fold, never on the
     full dataset. This mirrors Section 3.6's leakage-free design and is
     why the selected feature count still varies fold to fold (compare the
     printed per-fold counts to the 60-64 range already reported in Table 6).
  2. Hyperparameters are tuned via RandomizedSearchCV using an inner 5-fold
     StratifiedGroupKFold, exactly as in Section 3.6, using the search space
     in Table 7 (Random Forest block).
  3. SHAP values for each test fold come from the TreeExplainer built on
     that fold's fitted model — i.e., a model that has NEVER seen those
     samples in training. Concatenating across all 10 folds gives one
     out-of-fold SHAP value per (sample, feature) pair for the whole
     dataset — this is what is pooled for Figure 4.
  4. Because the Mann-Whitney feature set is not perfectly identical across
     folds, Figure 4 is built on the INTERSECTION of features selected in
     ALL 10 outer folds (documented in the figure caption). This keeps the
     pooled SHAP matrix rectangular and leakage-free without fabricating
     values for folds where a feature was not selected. The supplementary
     table separately reports every feature's selection frequency and rank
     range across folds, including features that were not selected in
     every fold.
  5. Figures 5 and 6 show one representative PD and one representative HC
     instance, each explained using the model and background distribution
     of the specific outer fold in which that instance was held out.
"""
